...
<div class="content-card p-3 p-lg-4">
    ...
    @if($isPending && $workflowRequest->currentStep)
        <div class="erp-approval-current-card mb-3">
            <div class="erp-approval-current-card__label">{{ __('ui.current_approval_step') }}</div>
            ...
            @foreach($approvalSteps as $step)
                @php
                    ...
                    $stepMeta = match ($stepState) {
                        'approved' => ['icon' => 'bi-check-circle-fill', 'label' => __('ui.approved')],
                        'pending' => ['icon' => 'bi-hourglass-split', 'label' => __('ui.approval_waiting')],
                        'returned' => ['icon' => 'bi-arrow-counterclockwise', 'label' => __('ui.returned')],
                        'rejected' => ['icon' => 'bi-x-circle-fill', 'label' => __('ui.rejected')],
                        'skipped_stock' => ['icon' => 'bi-skip-forward-fill', 'label' => __('ui.approval_skipped_stock')],
                        default => ['icon' => 'bi-circle', 'label' => __('ui.approval_not_started')],
                    };
                @endphp

                <div class="erp-approval-step erp-approval-step--{{ $stepState }}">
                    <div class="erp-approval-step__icon">
                        <i class="bi {{ $stepMeta['icon'] }}"></i>
                    </div>
                    <div class="flex-grow-1 min-w-0">
                        <div class="d-flex flex-wrap justify-content-between gap-2">
                            <div class="fw-semibold">{{ $step->step_name }}</div>
                            <span class="erp-approval-step__state">{{ $stepMeta['label'] }}</span>
                        </div>
                        <div class="small text-muted mt-1">
                            {{ $step->approverLabel() }}
                            @if($approval)
    ...
    <details class="border-top pt-3 erp-approval-activity" @if($isReturned || $isRejected) open @endif>
        <summary>{{ __('ui.approval_activity') }}</summary>
        <div class="mt-2">
