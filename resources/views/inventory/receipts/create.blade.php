    ...
    <div class="erp-form-layout">
        <section class="erp-form-section">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <label class="form-label erp-required">{{ __('inventory.receipt.product') }}</label>
                    <select name="product_id" class="form-select @error('product_id') is-invalid @enderror" required>
                        <option value="">{{ __('inventory.receipt.select_product') }}</option>
                        @foreach($products as $product)<option value="{{ $product->id }}" @selected((string)old('product_id') === (string)$product->id)>{{ $product->sku }} - {{ $product->name }}</option>@endforeach
                    </select>
